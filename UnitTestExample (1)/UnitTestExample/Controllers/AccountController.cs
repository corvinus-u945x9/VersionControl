...
namespace UnitTestExample.Controllers
{
    ...
    {        
        ...
        {
            var kisBetü = new Regex(@"[a-z]+");
            var nagyBetü = new Regex(@"[A-Z]+");
            var szam = new Regex(@"[0-9]+");
            var karakterhossz = new Regex(@".{8,}");
            return kisBetü.IsMatch(password) && nagyBetü.IsMatch(password) && szam.IsMatch(password) && karakterhossz.IsMatch(password);
        }
    }
}
